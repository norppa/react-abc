...
const eighthDown = ({ x, y }) => {
    const height = 4 * space
    const width = 2.16 * space
    x = x * space
    y = (y - 0.5) * space
    return (
        <svg x={x} y={y} width={width} height={height} viewBox="0 0 216 400">
            {quarterHead(0, 0)}
            {eighthDownFlag(12.45, 100)}
            <rect
                x="0"
                y="65"
                width="12.45"
...
const eighthUpFlag = (x, y) => {
    return (
        <svg x={x} y={y} width={0.9 * h} height={3 * h} viewBox="0 0 90 300">
            <path d="M 0.0574,1.1970497e-6 C 0.0574,4.4224312 0,7.6022712 0,11.522461 c 0,3.82021 0.25834,6.30059 0.61349,8.70135 4.14993,28.06732 10.1929,39.11592 40.36582,73.80263 C 79.13941,137.89514 90.22329,164.31898 89.99661,199.41 89.78583,232.32883 61.94397,302.83703 55.61358,299.9117 64.41608,275.24513 76.28614,248.63384 79.4432,226.36665 83.30137,199.15438 72.67209,160.74941 55.56602,140.48687 41.49851,123.8235 9.09743,108.91989 0.25915,108.91989 0.05736,1.1970498e-6 0.25915,108.91989 0.05736,1.1970498e-6 Z" />
        </svg>
    )
}

const eighthDownFlag = (x, y) => {
    return (
        <svg x={x} y={y} width={0.9 * h} height={3 * h} viewBox="0 0 90 300">
            <path d="M 0.0574,300 C 0.0574,295.57393 0,292.39147 0,288.46805 0,284.6447 0.25834,282.16227 0.61349,279.75954 4.76342,251.6691 10.80639,240.61141 40.97931,205.89613 79.13941,161.99131 90.22329,135.54571 89.99661,100.42579 89.78583,67.479853 70.429251,7.1750826 64.098861,10.102821 72.901361,34.789704 76.28614,51.161416 79.4432,73.446943 83.301371,100.68162 72.67209,139.11822 55.56602,159.39744 41.49851,176.07453 9.09743,190.99042 0.25915,190.99042 L 0.05736,300 Z" />
        </svg>
    )
}
...
export const note = (coord, { type, stem, accidental }) => {
    if (type === 'eighth' || type === '8th') {
        if (stem === 'down') return eighthDown(coord)
        else return eighthUp(coord)
    }

    if (type === 'quarter' || type === 'q') {
        if (stem === 'down') return quarterDown(coord)
        else return quarterUp(coord)
    }

    if (type === 'half' || type === 'h') {
        if (stem === 'down') return halfDown(coord)
        else return halfUp(coord)
    }
